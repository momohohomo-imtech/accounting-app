...
export async function createQualityChecklistItem(formData: FormData) {
  ...
  if (!values.project_id || !values.process_name || !values.item_name) {
    return { error: "프로젝트, 공정/공사명, 점검 항목을 입력해주세요." };
  }
...
export async function updateQualityChecklistItem(formData: FormData) {
  ...
  const values = parse(formData);
  if (!id || !values.project_id || !values.process_name || !values.item_name) {
    return { error: "프로젝트, 공정/공사명, 점검 항목을 입력해주세요." };
  }
