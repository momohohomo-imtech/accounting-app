...
1. **첨부파일 기반**(`attachments` 테이블, `project-files` storage 버킷,
   `supabase/040`, **실행 확인됨(2026-08-31, 사무실)** — 처음 실행 시 테이블이 이미
   존재해서 에러 났음(아마 이전에 부분 실행됨), `create table if not exists`/
   `drop policy if exists`로 고쳐서 재실행 완료함) — project_id와 work_date 둘 다 선택
   ...
   - `next.config.ts`에 서버 액션 기본 1MB 업로드 제한을 20MB로 올림 — 안 올리면
     사진 한 장도 못 올라감.
2. **견적서**(`quotes`/`quote_items` 테이블, `supabase/041`, **실행 확인됨
   (2026-08-31, 사무실)**) — `/projects`에 "견적서" 탭 하나만 추가(사이드바 무변경), 실제 다중 품목
...
화면에서 한 번도 못 눌러봄** — 빌드/타입체크/린트만 통과 확인됨. 마이그레이션
(040/041)은 2026-08-31 실행 확인됐지만, 실제 업로드/견적 작성/불러오기 흐름은
아직 사용자가 직접 확인 안 함 — 다음에 확인할 것.

## 이번 세션(집, 2026-09-01)에 한 일

사무실 세션 이후 33커밋 뒤처져 있던 로컬을 fast-forward pull로 동기화(로컬 변경
없어서 충돌 없음). `042_agency_purchase_memo.sql`/`043_project_settlement_finalized.sql`/
`044_agency_purchase_client_name.sql` 세 마이그레이션은 코드에는 이미 반영돼 있는데
HANDOFF에 실행 여부 기록이 없어서 사용자에게 확인 — **사용자가 이미 실행했다고
확인함**. 세 마이그레이션 모두 실행 완료로 기록.

**`/reports` 카테고리별 집계 개선** (DB 스키마 변경 없음, 전부 코드/UI):
- `CategoryAggregateTable.tsx`: "건수" 컬럼을 "매입 건수"/"대행구매 건수" 두 개로
  분리(각각 정렬 가능), 카테고리명이 링크가 되어 클릭하면 상세 팝업으로 이동
- `CategoryDetailReport.tsx`(신설, `VendorDetailReport.tsx`와 동일 패턴) — 매입
  내역과 대행구매 내역을 "구분" 컬럼으로 나눠 한 표에 합쳐 보여주고 헤더 클릭
  정렬 지원. 팝업 제목은 "{카테고리} 총 매입내역", 하단에 매입 합계·대행구매액
  합계를 나란히 표시(두 금액은 기존 컨벤션대로 절대 합산 안 하고 별도 표시).
  `CategoryReportActions.tsx`(신설)로 인쇄/엑셀 다운로드 지원.
- `reports/page.tsx`에 `category` 쿼리 파라미터 추가, 대행구매 조회 select에
  `id`/`item_name`/`client_name`/`memo`/프로젝트명 추가해서 상세 팝업에 필요한
  데이터를 서버에서 조합해 내려줌.
...
**후속: 품질관리를 하자보수 이력으로 재정의 + 노하우(know-how) 기능 추가**
(같은 세션 이어서, 사용자 요청):
- 품질관리 탭 부제목을 "공정/공사별 체크리스트" → "공정/공사별 하자보수관리
  및 이력"으로 변경(`QualityChecklistSection.tsx`, 텍스트만 — 필드/옵션은
  안 건드림. 처리 상태를 접수/처리중/완료 같은 하자보수 전용 상태로 바꾸는 건
  요청 범위 밖이라 보류, 필요하면 나중에 다시 요청받을 것).
- **노하우**: `KnowHowSection.tsx`(신규, 공용 컴포넌트) — 제목을 누르면
  아코디언으로 펼쳐지며 내용/메모를 보여주고, 그 안에서 바로 수정 가능. 상단
  "+ 노하우 추가" 버튼으로 새 항목 작성. 품질관리/공사관리/공구리스트 3개 탭
  전부 하단에 배치, `know_how_notes` 테이블 하나를 `category`
  컬럼(quality/construction/tools)으로 구분해서 공용으로 씀 — **프로젝트에
  안 묶임(공용 지식/요령)**, 그래서 프로젝트를 선택 안 한 상태에서도 항상
  보임(품질관리/공사관리는 프로젝트 선택 전엔 체크리스트 자체는 안 보이지만
  노하우 섹션은 별개로 항상 노출).
- `supabase/046_know_how_notes.sql` — 신규 테이블 1개, 019번과 동일한
  admin_staff RLS. **실행 여부 확인할 것.**
