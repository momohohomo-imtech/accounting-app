...
1. **결제방식 드롭다운 필터** (DB 스키마 변경 없음): `PaymentMethodFilter.tsx`
   ...
   서버에서 필터링됨.
2. **"프로젝트명이 전부 일반경비"는 버그가 아님** — 실제로 `project_id`가
   비어있는 거래라 그런 것(예: AI 엑셀 일괄등록으로 방금 넣은 거래들은
   프로젝트 미지정 상태로 들어옴). "일반경비"는 프로젝트 미연결을 일부러
   빨간 글씨로 강조하는 기존 설계(각 거래 "수정"에서 프로젝트를 지정하면
   실제 이름이 뜸) — 코드 변경 안 하고 사용자에게 설명함. 다음에 다시
   나오면 이 기록 참고할 것.
- 미검증(로그인 세션 없음), 빌드/린트만 통과.
